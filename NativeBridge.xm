...
static BOOL IsWA(NSString *s) {
    return [s isKindOfClass:[NSString class]] &&
           ([s isEqualToString:@"net.whatsapp.WhatsApp"] || [s isEqualToString:@"net.whatsapp.WhatsAppSMB"]);
}
...
static NSString *BundleID(id o, int depth) {
    ...
    for (NSString *k in @[@"sectionIdentifier",@"sectionID",@"bundleIdentifier",@"bundleID",@"applicationBundleIdentifier"]) {
        id v = Get(o,k);
        if ([v isKindOfClass:[NSString class]] && IsWA(v)) return v;
    }
    for (NSString *k in @[@"bulletin",@"bulletinRequest",@"notificationRequest",@"request",@"content"]) {
        id n = Get(o,k);
        if (n && n != o) {
            NSString *v = BundleID(n,depth+1);
            if (v) return v;
        }
    }
    return nil;
...
static id ScheduleID(id request) {
    ...
    if (h && [h respondsToSelector:decode]) {
        for (id o in @[b ?: [NSNull null], request ?: [NSNull null]]) {
            ...
            id c = Get(o,@"context");
            if (!c) continue;
            @try { sid = FindID(((id(*)(id,SEL,id))objc_msgSend)(h,decode,c),0); }
...
static NSString *FKey(id sid, NSString *bundle) {
    return (sid && bundle.length) ? [NSString stringWithFormat:@"%@|%@",bundle,[sid description]] : nil;
}

static void Record(id sid, NSString *bundle) {
    NSString *k = FKey(sid,bundle);
    if (!k) return;
...
static BOOL Recent(id sid, NSString *bundle) {
    ...
    @synchronized(lockObject) {
        ...
        for (NSString *old in [recent.allKeys copy]) {
            if ([now timeIntervalSinceDate:recent[old]] > 8.0) [recent removeObjectForKey:old];
        }
        NSDate *d = recent[k];
        return d && [now timeIntervalSinceDate:d] <= 8.0;
    }
}
...
static void InstallObserver(void) {
    ...
    if (h && [h respondsToSelector:s]) {
        %init(ObserveWatusi);
        observerInstalled = YES;
        Log(@{@"event":@"observer-ready"});
        return;
    }
...
static void Bridge(id request, NSString *hook) {
    NSString *bundle = BundleID(request,0);
    if (!IsWA(bundle)) return;
    id sid = ScheduleID(request);
    id b = Bulletin(request);
    if (!sid) {
        Log(@{@"event":@"whatsapp-notification",@"hook":hook ?: @"unknown",@"result":@"no-schedule-id",@"requestClass":NSStringFromClass([request class]) ?: @"unknown",@"requestID":RequestID(request) ?: @"unknown"});
        return;
    }
    if (Recent(sid,bundle)) {
        Log(@{@"event":@"schedule",@"hook":hook ?: @"unknown",@"result":@"already-forwarded",@"scheduleID":[sid description]});
        return;
    }
    Class h = NSClassFromString(@"WSSchedulerHelper");
    ...
    if (b && h && [h respondsToSelector:check]) {
        @try { ((id(*)(id,SEL,id))objc_msgSend)(h,check,b); }
        @catch (__unused NSException *e) {}
        if (Recent(sid,bundle)) {
            Log(@{@"event":@"schedule",@"hook":hook ?: @"unknown",@"result":@"checkBulletin-forwarded",@"scheduleID":[sid description]});
            return;
        }
    }
    if (h && [h respondsToSelector:send]) {
        @try {
            ((void(*)(id,SEL,id,id))objc_msgSend)(h,send,sid,bundle);
            Log(@{@"event":@"schedule",@"hook":hook ?: @"unknown",@"result":@"sendPush-fallback",@"scheduleID":[sid description]});
            return;
        } @catch (__unused NSException *e) {}
    }
...
- (void)postNotificationRequest:(id)request {
    %orig;
    Bridge(request,@"CSNotificationDispatcher");
}
...
- (void)postNotificationRequest:(id)request forCoalescedNotification:(id)coalesced {
    %orig;
    Bridge(request,@"SBDashBoardNotificationDispatcher");
}
