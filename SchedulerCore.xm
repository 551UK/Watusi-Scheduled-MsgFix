...
static NSString * const WSSMFVersion = @"1.0.13";
...
static NSString * const WSSMFHelperDebugPath = @"/var/mobile/Library/Preferences/com.551.watusischeduledmsgfix-helper-dedupe.plist";
...
static NSObject *gHelperLock;
static BOOL gHelperHookInstalled = NO;
...
static BOOL WSSMFClaimHelperDispatch(id scheduleID, NSString *bundleID, NSString *source) {
    ...
    WSSMFEnsureHelperState();

    NSString *key = [NSString stringWithFormat:@"%@|%@", bundleID, [scheduleID description]];
    NSDate *now = [NSDate date];
    BOOL allow = YES;

    @synchronized (gHelperLock) {
        for (NSString *oldKey in [gRecentHelperDispatches.allKeys copy]) {
            NSDate *oldDate = gRecentHelperDispatches[oldKey];
            if (!oldDate || [now timeIntervalSinceDate:oldDate] > WSSMFHelperDedupeWindow) {
                [gRecentHelperDispatches removeObjectForKey:oldKey];
            }
        }
    ...
    } writeToFile:WSSMFHelperDebugPath atomically:YES];
...
static void WSSMFScanSchedules(NSString *reason) {
    ...
    for (NSDictionary *store in WSSMFScheduleStores()) {
        ...
        for (id object in schedules) {
            ...
            if (!key.length || WSSMFWasDispatched(key)) continue;

            NSString *result = nil;
            BOOL posted = WSSMFFire(scheduleID, bundleID, &result);
            if (posted) WSSMFMarkDispatched(key);
...
static void WSSMFStartScheduler(void) {
    ...
    dispatch_async(gSchedulerQueue, ^{
        WSSMFScanSchedules(@"scheduler-start");
    });
}
...
static void WSSMFTryInstallHelperHook(void) {
    if (gHelperHookInstalled) return;

    Class helper = NSClassFromString(@"WSSchedulerHelper");
    SEL selector = NSSelectorFromString(@"sendPushNotificationForScheduleID:bundleIdentifier:");
    if (helper && [helper respondsToSelector:selector]) {
        %init(WSSMFHelperDedupe);
        gHelperHookInstalled = YES;
        WSSMFWriteDebug(@{@"event":@"helper-dedupe-hook",@"result":@"installed"});
        return;
    }

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 500 * NSEC_PER_MSEC), dispatch_get_main_queue(), ^{
        WSSMFTryInstallHelperHook();
    });
}
