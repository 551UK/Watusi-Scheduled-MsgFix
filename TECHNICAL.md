...
Before the first send, submission intent is persisted. A Core Data save observer associates a newly inserted outgoing object by chat session and text and saves its permanent object URI. Retries resolve that exact object and call retrySendingMessage:. Every recipient must satisfy Watusi's send-status predicate before the schedule completes. Deleting a schedule removes its ledger entry when WhatsApp next scans its schedule manager.
